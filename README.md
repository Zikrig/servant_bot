...
Telegram-бот для автоответчиков в Telegram Business чатах.
...
- Отложенные автоответы по таймеру: если владелец не ответил сам, бот отправляет заготовленный текст в чат подключенного бизнес-аккаунта.
...
- Для автоответов используются `business_message` и `business_connection_id`.
...
Для реальной работы бот должен быть подключен к бизнес-аккаунту Telegram и получать `business_connection` / `business_message` апдейты.
...
- Включенный сценарий применяется в business-чатах владельца, который подключил бота к своему бизнес-аккаунту.
